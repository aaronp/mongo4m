  ...
  "RichMongoDatabase.createRequiredCollections" should {
    "createRequiredCollections" in {
      Schedulers.using { implicit scheduler =>
        ...
        withClue(
          "Our collections should now exist, so running our cold-observable a second time should have no effect") {
          val shouldBeEmpty: List[String] =
            obs.toListL.runSyncUnsafe(testTimeout)
