  ...
  "ReactiveMongo" should {
    "be able to return a stream of query results based on a stream of mongo/query tuples" in {
      Schedulers.use(Schedulers.computeAsync()) { implicit sched =>
        ...
        val collections: Observable[MongoCollection[Document]] = Observable.fromResource(connect.mongoDbResource).flatMap { db =>
        ...
        val task: CancelableFuture[Unit] = results.flatten.map(BsonUtil.fromBson).flatMap(Observable.fromTry).foreach { json =>
          val numberValue = json.hcursor.downField("record").downField("number").as[Int].getOrElse(sys.error("bug"))
          seen += numberValue
        }
        try {
