...
trait DbQueryTest extends BasePipelinesMongoSpec with GivenWhenThen {
    ...
    "apply the query, projection and sort criteria" in {
      Schedulers.use(Schedulers.compute()) { implicit sched =>
        ...
        val query = new DbQuery(
          ...
          ascending = false
        ).asJson.as[DbQuery].toTry.get // let's use a query marshalled/unmarshalled from json, just to prove our codecs work

        val populateNewCollection = connect.mongoDbResource.use { db =>
          IO {
            val collection = connect.settingsForCollection(s"test.dbquery${System.currentTimeMillis}").ensureCreated(db).futureValue
        ...
        val recordsFound = found.map(_.values.last).map(BsonUtil.bsonValueAsJson)
  ...
  "DbQuery" should {
    "marshal to and from json" in {
      ...
      val originalFilter: Bson = Filters.or(Filters.eq("foo", 2), Filters.eq("bar", 3))
      val query = new DbQuery(
    ...
    "be able to return a stream of query results based on a stream of mongo/query tuples" in {
      Schedulers.use(Schedulers.compute(executionModel = ExecutionModel.AlwaysAsyncExecution)) { implicit sched =>
        val collectionName = s"test.dbquery${System.currentTimeMillis}"

        val queries = Var[DbQuery](null: DbQuery)

        val collection: Observable[MongoCollection[Document]] = connect.collectionObservable(collectionName)
        ...
        Observable(1, 2, 3, 4).map(DbQueryTest.Record.apply).subscribe(head.asObserver[Record])
...
object DbQueryTest {
  ...
  object Record {
    implicit val encoder: ObjectEncoder[Record] = io.circe.generic.semiauto.deriveEncoder[Record]
    implicit val decoder: Decoder[Record]       = io.circe.generic.semiauto.deriveDecoder[Record]
  }
}
