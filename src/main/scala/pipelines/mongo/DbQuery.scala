...
final case class DbQuery(query: Option[Bson], projection: Option[Bson], sortFields: Seq[String], ascending: Boolean = true) {
...
object DbQuery {
  ...
  def apply(query: Bson): DbQuery = new DbQuery(query = Option(query), None, Nil)
